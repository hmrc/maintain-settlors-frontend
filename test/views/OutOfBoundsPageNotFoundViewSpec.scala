...
class OutOfBoundsPageNotFoundViewSpec extends ViewBehaviours {
  ...
  "OutOfBoundsPageNotFound view" must {
    ...
    val applyView: HtmlFormat.Appendable = view.apply(key)(fakeRequest, messages)
    ...
    "display the bullet point links with the correct text and hrefs" in {
      ...
      addBeneficiaryLink.text() mustBe messages(
        s"$messageKeyPrefix.bullet2",
        messages(s"$messageKeyPrefix.bullet2.$key")
      )

      addBeneficiaryLink.attr("href") mustBe
        controllers.routes.AddASettlorController.onPageLoad().url
    }
  }
