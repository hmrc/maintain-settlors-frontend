    ...
    "generate deceased settlor section for all possible data" in {
      ...
      result mustBe AnswerSection(
        ...
        rows = Seq(
          ...
          AnswerRow(label = Html(messages("deceasedSettlor.nationalInsuranceNumber.checkYourAnswersLabel", name.displayName)), answer = Html("AA 00 00 00 A"), changeUrl = Some(controllers.individual.deceased.routes.NationalInsuranceNumberController.onPageLoad().url)),
