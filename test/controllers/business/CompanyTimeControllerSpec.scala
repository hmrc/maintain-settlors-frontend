...
class CompanyTimeControllerSpec extends SpecBase with MockitoSugar {
  ...
  "CompanyTime Controller" must {
    ...
    "return OK and the correct view for a GET" in {
      ...
      val request = FakeRequest(GET, companyTimeControllerRoute)

      val result = route(application, request).value

      val view = application.injector.instanceOf[CompanyTimeView]
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set(CompanyTimePage, true).success.value
    ...
    "redirect to the next page when valid data is submitted" in {
      ...
      val request =
        FakeRequest(POST, companyTimeControllerRoute)
