...
import pages.business.{LiveInTheUkYesNoPage, NamePage}
...
class LiveInTheUkYesNoControllerSpec extends SpecBase with MockitoSugar {
  ...
  val form: Form[Boolean] = formProvider.withPrefix("businessSettlor.liveInTheUkYesNo")
  val name                = "Name"
    ...
    "redirect to the next page when valid data is submitted" in {
      ...
      val application =
        applicationBuilder(userAnswers = Some(emptyUserAnswers))
          .overrides(bind[Navigator].qualifiedWith(classOf[BusinessSettlor]).toInstance(fakeNavigator))
          .build()
