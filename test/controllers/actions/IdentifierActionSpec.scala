  ...
  "invoking an AuthenticatedIdentifier" when {
    ...
    "the user hasn't logged in" must {
      "redirect the user to log in " in {
        ...
        redirectLocation(
          result
        ).get mustBe "http://localhost:9949/auth-login-stub/gg-sign-in?continue=http%3A%2F%2Flocalhost%3A9781%2Ftrusts-registration&origin=maintain-settlors-frontend"
        application.stop()
      }
    }
