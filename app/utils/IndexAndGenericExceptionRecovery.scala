...
trait IndexAndGenericExceptionRecovery {
  ...
  def outOfBoundsView: OutOfBoundsPageNotFoundView
    ...
    case _: IndexOutOfBoundsException =>
      logger.warn(
        s"[Session ID: ${utils.Session.id(hc)}][UTR/URN: $identifier] " +
          s"${classNameAndMethod(callingMethod)}: no $trusteeType at index $index, IndexOutOfBoundsException - showing page not found"
      )

      Future.successful(NotFound(outOfBoundsView(trusteeType)))
    case e                            =>
      logger.error(
        s"[Session ID: ${utils.Session.id(hc)}][UTR/URN: $identifier] " +
