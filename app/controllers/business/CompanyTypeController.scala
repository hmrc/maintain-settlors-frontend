...
import pages.business.CompanyTypePage
...
class CompanyTypeController @Inject() (
  override val messagesApi: MessagesApi,
  sessionRepository: PlaybackRepository,
  @BusinessSettlor navigator: Navigator,
  standardActionSets: StandardActionSets,
  nameAction: NameRequiredAction,
  formProvider: CompanyTypeFormProvider,
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = standardActionSets.verifiedForUtr.andThen(nameAction) {
    implicit request =>
      val preparedForm = request.userAnswers.get(CompanyTypePage) match {
        case None        => form
    ...
    standardActionSets.verifiedForUtr.andThen(nameAction).async { implicit request =>
      form
        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <- Future.fromTry(request.userAnswers.set(CompanyTypePage, value))
              _              <- sessionRepository.set(updatedAnswers)
            } yield Redirect(navigator.nextPage(CompanyTypePage, mode, updatedAnswers))
        )
    }
